...
let update (event : Event.t) (screen : section) =
  ...
  | Session_screen model ->
    if model.last_correct = String.length model.content
    then Screens.transition (Session_screen model), Command.Noop
    else (
...
let view screen =
  if screen.last_correct = String.length screen.content
  then "You're done!"
  else (
